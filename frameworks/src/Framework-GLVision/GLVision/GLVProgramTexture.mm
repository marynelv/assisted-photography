...
@implementation GLVProgramTexture
...
-(id) init
{
    self = [super initWithVertexShaderFilename:@"texturedProjection" 
                        fragmentShaderFilename:@"texture"];
    if (self)
    {
        attr = new GLuint[PROG_TEXTURE_ATTR_NUM];
        uni = new GLuint[PROG_TEXTURE_UNI_NUM];
        memset(attr, 0, sizeof(GLuint)*PROG_TEXTURE_ATTR_NUM);
        memset(uni, 0, sizeof(GLuint)*PROG_TEXTURE_UNI_NUM);
        ...
        NSLog(@"Program texture: %u", program);
    }
    return self;
}
...
{    
    ...
    // attributes
    attr[PROG_TEXTURE_ATTR_POSITION] = [self attributeIndex:"position"];
    //glEnableVertexAttribArray(attr[PROG_TEXTURE_ATTR_POSITION]);
    attr[PROG_TEXTURE_ATTR_TEXCOORD] = [self attributeIndex:"texCoordIn"];
    //glEnableVertexAttribArray(attr[PROG_TEXTURE_ATTR_TEXCOORD]);
    NSLog(@"(setup!) attr_pos=%u | attr_texcoord=%u", attr[PROG_TEXTURE_ATTR_POSITION], attr[PROG_TEXTURE_ATTR_TEXCOORD]);
    
    // uniforms
    uni[PROG_TEXTURE_UNI_PROJECTION] = [self uniformIndex:"projection"];
    uni[PROG_TEXTURE_UNI_TEXTURE] = [self uniformIndex:"texture"];
    NSLog(@"(setup!) uni_projection=%u | uni_tex=%u", uni[PROG_TEXTURE_UNI_PROJECTION], uni[PROG_TEXTURE_UNI_TEXTURE]);
...
{
    glDisableVertexAttribArray(attr[PROG_TEXTURE_ATTR_POSITION]);
    glDisableVertexAttribArray(attr[PROG_TEXTURE_ATTR_TEXCOORD]);
...
{
    if (index < 0 || index > PROG_TEXTURE_ATTR_NUM)
        return 0;
    return attr[index];
}
...
{
    if (index < 0 || index > PROG_TEXTURE_UNI_NUM)
        return 0;
    return uni[index]; 
}
